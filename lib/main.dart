...
Future<void> main() async {
  ...
  try {
    ...
    runApp(AppStartupFailureApp(message: error.toString()));
  }
}

/// Tampilan aman saat konfigurasi build belum diberikan.
...
class AppStartupFailureApp extends StatelessWidget {
  final String message;

  const AppStartupFailureApp({required this.message, super.key});
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    const Text(
                      'Periksa konfigurasi Firebase, Supabase, dan koneksi '
                      'perangkat, lalu jalankan kembali aplikasi.',
...
/// Root widget aplikasi produksi E-Ticketing Helpdesk.
                ...
                listener: (context, state) {
                  ...
                  if (state.status == AuthStatus.authenticated) {
                    context.read<TicketListBloc>().add(
                          const list_event.StartTicketListSubscription(),
                        );
                    context
                        .read<NotificationBloc>()
                        .add(StartNotificationSubscription());
                    context.read<TicketStatsBloc>().add(
                          const stats_event.FetchTicketStatsRequested(),
                        );
                  }
                  ...
                  if (state.status == AuthStatus.unauthenticated ||
                    ...
                    context.read<TicketListBloc>().add(
                          list_event.ResetTicketListState(),
                        );
                    context.read<TicketStatsBloc>().add(
                          stats_event.ResetTicketStatsState(),
                        );
                    context
                        .read<NotificationBloc>()
